...
@if($data->gateway)
<div class="token-overview"><div class="row"><div class="col-md-6 col-sm-6"><div class="token-bonus token-bonus-sale"><span class="token-overview-title">Payment Method</span><span class="token-overview-value text-primary">Credit/Debit Card</span></div></div><div class="col-md-6 col-sm-6"><div class="token-bonus token-bonus-samount"><span class="token-overview-title">Payment Gateway</span><span class="token-overview-value text-primary">{{App\Gateway::whereId($data->gateway)->first()->name}}</span></div></div> </div></div>

@else
<div class="token-overview"><div class="row"><div class="col-md-4 col-sm-6"><div class="token-bonus token-bonus-sale"><span class="token-overview-title">{{App\Bank::whereId($data->bank)->first()->name}}</span><span class="token-overview-value text-primary">Bank Name</span></div></div><div class="col-md-4 col-sm-6"><div class="token-bonus token-bonus-samount"><span class="token-overview-title">{{App\Bank::whereId($data->bank)->first()->accname}}</span><span class="token-overview-value text-primary">Account Name</span></div></div><div class="col-md-4"><div class="token-total"><span class="token-overview-title font-bold">{{App\Bank::whereId($data->bank)->first()->account}}</span><span class="token-overview-value schedule-titl text-primary">Account Number</span></div></div></div></div>
@endif


<div class="note note-plane note-danger note-sm pdt-1x pl-0"><p>Do not pay below {{$basic->currency_sym}}{{number_format($data->main_amo, $basic->decimal)}} After payment, click Confirm Payment button below and fill/upload your payment information.</p></div></div>




 <div class="pdb-1x"><h5 class="schedule-title"><span>Amount Payable</span></h5><span class="schedule-title text-secondary">
 @if($data->gateway == 103)
 USD{{number_format($data->main_amo/$basic->rate, $basic->decimal)}}
 @else
 {{$basic->currency_sym}}{{number_format($data->main_amo, $basic->decimal)}}
 @endif
 </span><div class="pdb-1x"><h5 class="schedule-title"><span>What You Get</span></h5><span class="schedule-title text-secondary">
...
</div><!-- .input-item --><ul class="d-flex flex-wrap align-items-center guttar-30px"><li><button type="submit" class="btn btn-primary">Confirm Payment</button></form></li></ul><div class="gaps-2x"></div><div class="gaps-1x d-none d-sm-block"></div><div class="note note-plane note-light mgb-1x"><em class="fas fa-info-circle"></em><p>You will be credited {{$data->getamo}}  once your payment is confirmed.</p></div><div class="note note-plane note-danger"><em class="fas fa-info-circle"></em><p>In case you send a different amount, please contact {{$basic->sitename}}, we will update accordingly.</p></div></div></div><!-- .modal-content --></div><!-- .modal-dialog --></div><!-- Modal End -->
